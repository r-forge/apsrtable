...
setGeneric("apsrtableSummary", function(obj, ...) {
    standardGeneric("apsrtableSummary") })

apsrtableSummary.mer <- function (obj, alpha = lev, ...) {
  s <- summary(obj)
  fcoef <- coef(s)
...
"apsrtableSummary.polr" <- function (x) {
       ...
       newCoef <- coef(s)
       newCoef[,4] <- newCoef[,3]
       newCoef[,3] <- pt(abs(coef(s)[,3]),
                         df=s$n-s$edf-1,
                         lower.tail=FALSE)
       colnames(newCoef) <- c("coef","se(coef)", "Pr(>|t|)", "t value")
       s$coefficients <- newCoef
       return(s)
}
